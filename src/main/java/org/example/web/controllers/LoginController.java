...
public class LoginController {
    ...
    @PostMapping("/auth")
    public String authenticate(LoginForm loginForm, Model model) throws BookShelfLoginException {
    ...
    public String register(LoginForm loginForm, Model model){
        if(loginForm.getUsername().isEmpty() || loginForm.getPassword().isEmpty()){
            logger.info("login FAIL redirect back to login");
            model.addAttribute("registerErrorMessage", "All fields must be filled");
            return login(model);
        }
        if(loginService.register(loginForm)){
            ...
            logger.info("login FAIL redirect back to login");
            model.addAttribute("registerErrorMessage", "User with the same name already exists");
            return login(model);
        }
    }
